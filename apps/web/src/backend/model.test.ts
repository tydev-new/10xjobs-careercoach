...
import { authedFetch, COACH_MODEL_ID, createCoachModel, NO_DATA_KEPT_PROVIDER_FILTER } from "./model.ts";
...
test("createCoachModel: model id and the no-data-kept provider filter are on the request body", async () => {
  const { fetchImpl, calls } = makeStubFetch();
  const model = createCoachModel({
    proxyUrl: "https://proj.supabase.co/functions/v1/ten-model-proxy",
  ...
  assert.equal(body.model, COACH_MODEL_ID);
  assert.deepEqual(body.provider, NO_DATA_KEPT_PROVIDER_FILTER);
  assert.deepEqual((body as any).cache_control, { type: "ephemeral" });
