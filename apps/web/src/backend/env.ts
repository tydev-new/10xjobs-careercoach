...
// VITE_SITE_URL (read by backend/auth.ts's own siteRedirectUrl — not
// duplicated here), VITE_MODEL_PROXY_URL (default derived from the Supabase
...
export function readEnv(source: Record<string, string | undefined>): TenEnv {
  ...
  return { supabaseUrl: supabaseUrl!.replace(/\/+$/, ""), supabaseAnonKey: supabaseAnonKey!, modelProxyUrl };
}
